...
def data_prepare(sig_file, bkg_file, var_order, var_obt, verbose=0, **kwargs):
    """
    Input: 
    ...
    **kwargs: additional requirement to slim input samples: {'variable': 'requirement'}. e.g. {'nBtag_85':'>=4'}
    ...
    X_sig = X_sig.reshape((n_evt_sig,nb_combinations, n_var))
    ...
    X_bkg = X_bkg.reshape((n_evt_bkg, nb_combinations, n_var))
    print("bkg X shape: ")
    print X_bkg.shape
...
def aux_input(input_dict, input_list):
    aux_feature = []
    for istr in input_list:
        aux_feature.append(input_dict[istr])
    aux_feature = np.array(aux_feature)
    aux_feature = aux_feature.T
    return aux_feature
